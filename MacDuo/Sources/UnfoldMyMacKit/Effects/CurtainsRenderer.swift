...
@MainActor final class CurtainsPipeline: MetalEffectPipeline {
    ...
    init() throws {
        ...
        guard let url = Bundle.module.url(forResource: "Curtains", withExtension: "metal", subdirectory: "Shaders") else {
        ...
        guard let vertices = gpu.makeBuffer(bytes: points, length: points.count * MemoryLayout<SIMD2<Float>>.stride),
            ...
            throw NSError(domain: AppIdentity.name, code: 4, userInfo: [NSLocalizedDescriptionKey: "Unable to create the curtain mesh."])
        }
