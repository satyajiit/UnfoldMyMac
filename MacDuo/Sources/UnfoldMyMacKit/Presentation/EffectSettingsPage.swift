    ...
    private var lidTiming: some View {
        ContentCard {
            VStack(alignment: .leading, spacing: 20) {
                ...
                ParameterRow(title: "Activate at or below", valueLabel: "\(Int(model.settings.activation))°", value: Binding(get: { model.settings.activation }, set: { model.setActivation($0) }), range: 60...180, step: 1)
                HStack {
                ...
                Divider()
                ParameterRow(title: "Finish effect by", valueLabel: "\(Int((model.settings.completionFraction * 100).rounded()))% of lid travel", value: Binding(get: { model.settings.completionFraction }, set: { model.setCompletionFraction($0) }), range: 0.4...1)
