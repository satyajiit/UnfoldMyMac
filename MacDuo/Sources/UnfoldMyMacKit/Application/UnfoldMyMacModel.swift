...
enum EffectsDestination: Hashable { case settings }
...
@MainActor @Observable public final class UnfoldMyMacModel: NSObject {
    ...
    func setActivation(_ angle: Double) {
        ...
        settings.activation = min(180, max(60, angle)).rounded()
        persist(); tick()
    }
    func setCompletionFraction(_ fraction: Double) { settings.completionFraction = min(1, max(0.4, fraction)); persist() }
