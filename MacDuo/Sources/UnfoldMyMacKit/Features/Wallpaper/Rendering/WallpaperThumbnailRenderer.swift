...
@MainActor enum WallpaperThumbnailRenderer {
    static func cover(pipeline: WallpaperPipeline) throws -> NSImage {
        if let name = pipeline.template.coverImage,
           let url = BundleResources.wallpaperCover(name),
    ...
    static func image(pipeline: WallpaperPipeline, size: CGSize = CGSize(width: 640, height: 400)) throws -> NSImage {
        let scale = min(1, GPUTuning.maximumWallpaperDimension / max(1, max(size.width, size.height)))
        ...
        guard let texture = pipeline.catalog.gpu.makeTexture(descriptor: descriptor),
              let command = pipeline.catalog.queue.makeCommandBuffer() else { throw GPUError.allocationFailed("the thumbnail surface") }
        ...
        guard pipeline.encode(command: command, pass: pass, size: CGSize(width: width, height: height), time: 4, energy: 0.35) else { throw GPUError.allocationFailed("the thumbnail encoder") }
        ...
        guard let provider = CGDataProvider(data: Data(pixels) as CFData), let color = CGColorSpace(name: CGColorSpace.sRGB),
              let image = CGImage(width: width, height: height, bitsPerComponent: 8, bitsPerPixel: 32, bytesPerRow: width*4,
                ...
                provider: provider, decode: nil, shouldInterpolate: true, intent: .defaultIntent) else { throw GPUError.allocationFailed("the thumbnail image") }
