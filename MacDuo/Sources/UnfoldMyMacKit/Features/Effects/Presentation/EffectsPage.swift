            ...
            Section {
                VStack(alignment: .leading, spacing: 20) {
                    VStack(spacing: 12) {
                        HStack(spacing: 12) {
                            HStack(spacing: 8) {
                                IconGlyph(icon: .search, size: 14).foregroundStyle(palette.secondary)
                                TextField("Search designs, tags, authors", text: $model.libraryQuery,
                                          prompt: Text("Search designs, tags, authors").foregroundStyle(palette.secondary))
                                ...
                                if !model.libraryQuery.isEmpty {
                                    Button { model.libraryQuery = "" } label: { Label("Clear search", icon: .clear) }
                                        .labelStyle(.iconOnly).buttonStyle(.plain).foregroundStyle(palette.secondary)
                                }
