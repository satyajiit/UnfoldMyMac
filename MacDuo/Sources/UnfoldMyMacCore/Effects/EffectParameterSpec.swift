...
public struct EffectParameterSpec: Codable, Equatable, Sendable, Identifiable {
    ...
    public init(key: String, title: String, kind: Kind, range: ClosedRange<Double> = 0...1, step: Double = 0.01,
                format: Format = .percent, default: EffectParameterValue, choices: [Choice] = []) {
    ...
    public var isValid: Bool {
        guard !key.isEmpty, key.count <= 40, !title.isEmpty, [minimum, maximum, step].allSatisfy(\.isFinite) else { return false }
        switch kind {
    ...
    private enum CodingKeys: String, CodingKey { case key, title, kind, range, step, format, `default`, choices }
