#!/usr/bin/env bash
# Precompiles every shader unit the app declares into <bundle>/Shaders/Compiled/<digest>.metallib.
# The digest is the SHA-256 of the exact text ShaderLibraryCache compiles plus its math mode, so the
# runtime finds a unit by content and falls back to source compilation for anything else. Unit files are
# bundle-relative: shared modules under Shaders/, template-owned scenes under Resources/Wallpapers/<id>/.
set -euo pipefail
EXECUTABLE="${1:?usage: compile_shaders.sh <executable> <resource-bundle-dir>}"
BUNDLE_DIR="${2:?usage: compile_shaders.sh <executable> <resource-bundle-dir>}"
...
with tempfile.TemporaryDirectory() as tmp:
    for unit in units:
        ...
        for name in unit["files"]:
            path = pathlib.Path(name) if os.path.isabs(name) else bundle / name
            with open(path, encoding="utf-8", newline="") as handle: texts.append(handle.read())
        text = "\n".join(texts)
