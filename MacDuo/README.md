...
Thirteen lid-driven desktop effects, your own image art, and living wallpapers powered by real Mac data. Native macOS 26+, SwiftUI/AppKit, and Metal. No third-party dependencies.

Open **Wallpaper** for ten scenes: Pulse, Claude Current, Daydream, Codex Foundry, Grok Event Horizon, **GitHub After Hours**, **Codex Mission Control**, **Lights Out**, **GTA VI — Vice City Countdown**, and **Aurora Observatory**. The latest additions pair official Rockstar artwork and a calendar-day countdown with a 3D Earth driven by NOAA space weather; neither requires account setup. The collection also includes a floating city driven by your public GitHub profile, an expressive 3D robot driven by real Codex lifecycle hooks, and an animated F1 car/track with the original logo. Codex/Grok rotating headlines fit bounded two-line areas. Preview a scene, then choose **Use wallpaper** to apply it to all displays. Each card has **Set up / Configure** for its own profile, local usage, hooks and tool feeds. **Use wallpaper** opens required setup before applying; GitHub begins with an empty username field. **Playback settings** contains shared motion and background preferences. Codex hooks require a one-time review in Codex `/hooks`. Templates and providers are independently extensible; see [the Wallpaper engine guide](WALLPAPER_ENGINE.md).

```sh
./script/build_and_run.sh
```

The build produces `dist/UnfoldMyMac.app`. Open **Effects** to choose a design, preview any card, and turn lid effects on or off. Its header shows the selected design and an **Effect settings** action for lid timing, menu-bar angle, Screen Recording, and diagnostics. Use Back to return to the library. The large heading and controls scroll away, leaving only a compact page title in the native toolbar; scrolling to the top restores them. App-wide **Settings**, at the bottom of the sidebar, contains appearance, accessibility status, and app information. Each card has a full artwork cover, selection button, Preview action, and Adjust control. Preview plays directly on your desktop with floating Play/Pause, slider, and Stop controls; it keeps your chosen effect unchanged. Stop restores the previous enabled state. With Reduce Motion, preview starts in manual mode. UnfoldMyMac appears in the Dock and app switcher. Closing its window keeps the Dock icon and menu-bar controls available; click the Dock icon to reopen it.
...
**Effects** groups designs into **Glass & Light**, **Image Art**, and **Motion & 3D**, with cover thumbnails and author credits. Search titles, tags, and authors; use the tag filter for Game Art, Motivation, Energetic, Calm, and more. **Add image** imports a local image into Image Art. UnfoldMyMac keeps its own copy, so moving the original does not break the design. Adjust lets you rename it, credit its author, change its reveal, or remove UnfoldMyMac’s copy. Imports accept supported images up to 50 MB and 200 MP, normalized to a 4096-pixel maximum; animated formats use the first frame. Covers are illustrative—Preview shows the rendered effect.
