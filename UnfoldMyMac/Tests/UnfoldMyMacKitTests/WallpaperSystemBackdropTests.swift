...
@Test(.requiresGPU, .tags(.gpu)) @MainActor func systemBackdropRefreshesEditedShadersAndKeepsOriginalRestoration() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: root) }
    let access = FakeDesktopImages(), original = access.images
    let catalog = WallpaperShaderCatalog(), gpu = try TestGPU.context()
    var template = try #require(WallpaperTemplateRegistry(shaders: catalog, loadUserTemplates: false).templates.first)
    template.shader = "backdrop-refresh"
    let backdrop = WallpaperSystemBackdrop(access: access, directory: root.appendingPathComponent("stills"))
    var keys = Set<String>(), urls = Set<URL>()
    for color in [".15,.30,.45", ".45,.30,.15"] {
        let folder = root.appendingPathComponent(color)
        try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true)
        try "fragment float4 backdropTestFragment() { return float4(\(color),1); }"
            .write(to: folder.appendingPathComponent("Scene.metal"), atomically: true, encoding: .utf8)
        try catalog.register(template.shader, scene: .init(source: "Scene.metal", fragment: "backdropTestFragment"), folder: folder)
        let pipeline = try WallpaperPipeline(template: template, gpu: gpu, shaders: catalog)
        keys.insert(pipeline.backdropKey)
        try backdrop.apply(pipeline)
        urls.insert(try #require(access.images["display"]).url)
    }
    #expect(keys.count == 2 && urls.count == 2 && backdrop.renders == 2)
    let restarted = WallpaperSystemBackdrop(access: access, directory: root.appendingPathComponent("stills"))
    try restarted.apply(WallpaperPipeline(template: template, gpu: gpu, shaders: catalog))
    #expect(restarted.renders == 0, "Unchanged shader stills remain reusable across launch")
    try restarted.restore()
    #expect(access.images == original)
}
