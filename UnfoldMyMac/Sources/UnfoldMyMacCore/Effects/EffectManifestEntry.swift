...
public struct EffectManifestEntry: Decodable, Sendable, Identifiable {
    ...
    public init(id: EffectID, title: String, subtitle: String, detail: String, symbol: String, category: EffectCategory,
                ...
                renderer: String, renderingLabel: String, capabilities: Capabilities = .init(), parameters: [EffectParameterSpec], metadata: ContentMetadata? = nil) {
    ...
    public var isValid: Bool {
        (metadata?.isValid ?? true) && !id.rawValue.isEmpty && id.rawValue.count <= 80 && !title.isEmpty && !renderer.isEmpty && !cover.isEmpty &&
    ...
    public func descriptor(coverURL: URL?, isImported: Bool = false) -> EffectDescriptor {
        .init(id: id, title: title, subtitle: subtitle, detail: detail, symbol: symbol, requiresCapture: capabilities.requiresCapture,
              ...
              isImported: isImported, hasContinuousMotion: capabilities.continuousMotion, parameters: parameters, metadata: metadata)
    }

    private enum CodingKeys: String, CodingKey {
        case id, title, subtitle, detail, symbol, category, tags, author, credit, cover, asset, renderer, renderingLabel, capabilities, parameters, metadata
    }
    public init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        metadata = try values.decodeIfPresent(ContentMetadata.self, forKey: .metadata)
