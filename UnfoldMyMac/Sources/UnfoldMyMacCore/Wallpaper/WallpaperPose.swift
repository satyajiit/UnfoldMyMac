...
public struct WallpaperPose: Equatable, Sendable {
    ...
    public var liveInputs = WallpaperLiveInputs()

    public init(energy: Double = 0, channels: SIMD4<Float> = .zero, grid: WallpaperScalarGrid? = nil, liveInputs: WallpaperLiveInputs = .init()) {
        self.energy = energy; self.channels = channels; self.grid = grid; self.liveInputs = liveInputs
    }
