...
@MainActor @Observable final class WallpaperSetupController {
    ...
    @ObservationIgnored let inputs: WallpaperInputService?
    ...
    init(preferences: any PreferencesStore, registry: WallpaperConnectorRegistry = .standard, inputs: WallpaperInputService? = nil) {
        self.inputs = inputs
    ...
    /// Sound toggles take effect while editing; Cancel restores the saved connection.
    func setDraft(_ value: WallpaperConnectionSettings, for connector: String) {
        draft[connector] = value
        inputs?.setConnections(effectiveConnections)
    }
    var effectiveConnections: [String: [String: WallpaperConnectionSettings]] {
        guard let request else { return connections }
        var preview = connections; preview[request.template.id] = draft
        return preview
    }
    func cancel() { request = nil; draft = [:]; inputs?.setConnections(connections) }
    func finish() {
