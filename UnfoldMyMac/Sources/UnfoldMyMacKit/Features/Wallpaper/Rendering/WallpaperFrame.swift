...
struct WallpaperFrame: Equatable, Sendable {
    ...
    init(time: Double = 0, energy: Double = 0, channels: SIMD4<Float> = .zero, grid: WallpaperScalarGrid? = nil, liveInputs: WallpaperLiveInputs = .init()) {
        self.time = time; self.energy = energy; self.channels = channels; self.grid = grid; self.liveInputs = liveInputs
    }
