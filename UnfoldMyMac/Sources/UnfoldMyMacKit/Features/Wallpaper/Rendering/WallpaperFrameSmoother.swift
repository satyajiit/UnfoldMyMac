...
struct WallpaperFrameSmoother: Equatable, Sendable {
    ...
    var targetLiveInputs = WallpaperLiveInputs()
    private var liveSmoother = WallpaperLiveInputSmoother()
    ...
    mutating func advance(delta: Double, animating: Bool) -> WallpaperFrame {
        ...
        let liveInputs = liveSmoother.advance(toward: targetLiveInputs, delta: delta, animating: animating)
        return WallpaperFrame(time: animating ? time : stillTime, energy: energy, channels: channels, grid: grid, liveInputs: liveInputs)
    }
}
