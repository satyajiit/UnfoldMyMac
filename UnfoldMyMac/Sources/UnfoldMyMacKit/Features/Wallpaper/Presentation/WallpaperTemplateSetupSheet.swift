    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(alignment: .leading, spacing: 4) {
                Text(request.template.title).font(.title2.weight(.semibold))
                Text(request.template.contentCollection == .scenes ? "Scene settings" : "Wallpaper settings").font(.subheadline).foregroundStyle(.secondary)
            }
            ...
            Divider()
            HStack(spacing: 12) {
                Text(request.template.setup?.contains(where: { $0.id == "garden" }) == true
