...
@MainActor @Observable final class WallpaperSurfaceModel {
    ...
    private(set) var styleSheet: WallpaperStyle = .standard
    ...
    func reset(template: WallpaperTemplate?, styleSheet: WallpaperStyle = .standard) {
        self.template = template; self.styleSheet = styleSheet; snapshot = WallpaperSnapshot(); stats = [:]
    }
