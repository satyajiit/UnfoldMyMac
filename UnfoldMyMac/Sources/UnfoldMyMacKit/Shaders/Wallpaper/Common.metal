...
// environment: battery, daylight, Reduce Motion, persistent external power.
struct WallpaperUniforms { float2 size; float time; float energy; float4 accent; float4 background; float4 channels; float4 params[2]; float4 interaction; float4 environment; float4 motion; };
