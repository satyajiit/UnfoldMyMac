...
@MainActor enum AppDiagnostics {
    static func probe() -> Bool {
        ...
        let reading = motion.read(now: ProcessInfo.processInfo.systemUptime)
        print(motion.diagnostic)
        if let gravity = reading.acceleration { print("Acceleration (g): \(gravity)") }
        if let rotation = reading.rotation { print("Gyroscope (degrees/s): \(rotation)") }
        motion.stop()
