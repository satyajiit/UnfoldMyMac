...
import { BatteryCharging, BatteryFull, Compass, Folder, MessageCircle, Music2, Search, Settings2, Terminal, Trash2, Wifi } from "lucide-react";
import styles from "./lid-demo.module.css";

export function MacDesktopChrome({ charging = false }: { charging?: boolean }) {
  return <div className={styles.desktopChrome} aria-hidden="true">
    <div className={styles.menuBar}><span className={styles.menuApp}>◈</span><strong>Finder</strong><span>File</span><span>Edit</span><span>View</span><span>Go</span><span className={styles.menuSpacer} /><Wifi />{charging ? <BatteryCharging /> : <BatteryFull />}<Search /><span>9:41</span></div>
