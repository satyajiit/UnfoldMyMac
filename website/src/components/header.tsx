  ...
  return <header className="site-header">
    <div className="header-inner">
      ...
      <nav id="main-navigation" aria-label="Main navigation" className={open ? "main-nav is-open" : "main-nav"} onKeyDown={event => { if (event.key === "Escape") { setOpen(false); document.getElementById("menu-toggle")?.focus(); } }}>
        {navigation.map(item => <Link href={item.href} key={item.href} aria-current={pathname.startsWith(item.href) ? "page" : item.href === "/showcase/" && libraryCollections.some(collection => pathname.startsWith(`/${collection.slug}/`)) ? "location" : undefined} onClick={() => setOpen(false)}>{item.label}</Link>)}
      </nav>
